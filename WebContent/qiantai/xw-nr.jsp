	...
	<div class="center clearfix">
		...
		<div class="ab-right r clearfix">
			<div class="ab-right-t">
				<c:forEach var="newsCategory" items="${menu}">
					<c:if test='${news.cid==newsCategory.id}'>${newsCategory.name}</c:if>
				</c:forEach>
			</div>
			...
			<div class="ab-right-nr clearfix">
				<div class="zx_news_wz">
					<ul>
						<LI
							style="text-align: center; font-size: 24px; margin-bottom: 15px;"><h1>${news.title}
							</h1></LI>
						<LI style="text-align: center; background-color: #e7e7e7;">
							发布者：${news.author} 发布时间:${news.updatetime }</LI>
						<LI class="wznr">${news.content}</LI>
					</ul>
				</div>
				<div class="zx_wz_ys">
					<c:if test="${prenews!=null }">
						<span class="red_12">+ 上一篇：</span>
						<a
							href="${pageContext.request.contextPath}/NewsDetailServlet?id=${prenews.id}">${prenews.title}</a>
						<br />
					</c:if>
					<c:if test="${nextnews!=null }">
						<span class="red_12"> + 下一篇：</span>
						<a
							href="${pageContext.request.contextPath}/NewsDetailServlet?id=${nextnews.id}">${nextnews.title}</a>
				</div>
				</c:if>
