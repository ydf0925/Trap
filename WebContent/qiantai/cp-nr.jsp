...
<body>
	...
	<div class="center clearfix">
		<div class="ab-left l">
			<div class="ab-left-t">
				<div class="ab-left-t2">案例展示</div>
				<div class="ab-left-nav clearfix">
					<ul>
						<c:forEach items="${menu}" var="m">
							<LI><a
								href="${pageContext.request.contextPath}/CaseShowServlet?page=1&rows=5000&cp=1&cid=${m.id}">${m.name}</a></LI>
						</c:forEach>
		...
		<div class="ab-right r clearfix">
			<div class="ab-right-t">
				<c:forEach var="caseShow" items="${menu}">
					<c:if test='${cases.cid==caseShow.id}'>${caseShow.name}</c:if>
				</c:forEach>
			</div>
			...
			<div class="ab-right-nr clearfix">
				<div id="show_12">
					<div id="case_post_01">
						<img src="/imagespath/${cases.imgname}" width="450"
							height="302">
					</div>
					<div id="case_post_02">${cases.title}</div>
					<div id="case_post_03">${cases.content}</div>
				</div>
				<div id="case_post_04">
					<c:if test="${prencase!=null}">
						<span class="red_12">+ 上一例：</span>
						<a
							href="${pageContext.request.contextPath}/CaseDetailServlet?id=${prencase.id}">${prencase.title}</a>
						<br>
					</c:if>
					<c:if test="${nextcase!=null}">
						<span class="red_12">+ 下一例：</span>
						<a
							href="${pageContext.request.contextPath}/CaseDetailServlet?id=${nextcase.id}">${nextcase.title}</a>
				</div>
				</c:if>
