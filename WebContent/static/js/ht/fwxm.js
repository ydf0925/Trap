...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelFwxmServlet",
        type: "POST",
        data: jsondata,
        success: function (data, stutas) {
            ...
            if (data == "1") {
                ...
                successalert("", "删除服务项目成功");
            } else {
                errorsalert("", "删除服务项目失败");
            }
        },
        error: function () {
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateFwxmServlet",
        type: "POST",
        data: $("#changefrom").serialize(),
        success: function (data) {
            if (data == 1) {
                successalert("", "服务项目修改成功");
                $("#table_list_2").trigger("reloadGrid");
                changedialogdismiss();
            } else {
                errorsalert("", "出现错误，请重试");
...
function changedialogshow(id) {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/FwxmServlet",
        type: "POST",
    ...
    $("#table_list_2").jqGrid(
        {
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddFwxmServlet",
            hidegrid: false,
