...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelNewsCategoryServlet",
        type: "POST",
        data: jsondata,
        success: function (data, stutas) {
            ...
            if (data == "1") {
                ...
                successalert("", "删除新闻分类成功");
            } else {
                errorsalert("", "删除新闻分类失败");
            }
        },
        error: function () {
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateNewsCategoryServlet",
        type: "POST",
        data: $("#changefrom").serialize(),
        success: function (data) {
            if (data == 1) {
                successalert("", "新闻分类修改成功");
                $("#table_list_2").trigger("reloadGrid");
                changedialogdismiss();
            } else {
                errorsalert("", "出现错误，请重试");
    ...
    $("#table_list_2").jqGrid(
        {
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddNewsCategoryServlet",
            hidegrid: false,
