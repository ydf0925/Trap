...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelDeviceShowServlet",
        type: "POST",
        data: jsondata,
        success: function (data, stutas) {
            ...
            if (data == "1") {
                ...
                successalert("", "删除设备分类成功");
            } else {
                errorsalert("", "删除设备分类失败");
            }
        },
        error: function () {
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateDeviceShowServlet",
        type: "POST",
        data: $("#changefrom").serialize(),
        success: function (data) {
            if (data == 1) {
                successalert("", "设备分类修改成功");
                $("#table_list_2").trigger("reloadGrid");
                changedialogdismiss();
            } else {
                errorsalert("", "出现错误，请重试");
    ...
    $("#table_list_2").jqGrid(
        {
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddDeviceShowServlet",
            hidegrid: false,
