...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelAboutServlet",
        type: "POST",
        data: jsondata,
        success: function (data, stutas) {
            ...
            if (data == "1") {
                ...
                successalert("", "删除公司简介成功");
            } else {
                errorsalert("", "删除公司简介失败");
            }
        },
        error: function () {
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateAboutServlet",
        type: "POST",
        data: $("#changefrom").serialize(),
        success: function (data) {
            if (data == 1) {
                successalert("", "公司简介修改成功");
                $("#table_list_2").trigger("reloadGrid");
                changedialogdismiss();
            } else {
                errorsalert("", "出现错误，请重试");
...
function changedialogshow(id) {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/AboutServlet",
        type: "POST",
    ...
    $("#table_list_2").jqGrid(
        {
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddAboutServlet",
            hidegrid: false,
