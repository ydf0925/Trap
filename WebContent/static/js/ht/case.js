...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelCaseServlet",
        type: "POST",
        data: jsondata,
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateCaseServlet",
        type: "POST",
...
function buildgrid() {
    ...
    $("#table_list_2").jqGrid(
        {
            url: $("#PageContext").val()+"/CaseServlet",
            datatype: "json",
            mtype: "POST",
            height: "100%",
            autowidth: true,
            ...
            rowNum: 10,
            rowList: [10, 20, 30],
            colNames: ['案例标题','分类名称', '更新日期','案例内容', '图片预览', '操作'],
            colModel: [
                {
                    name: 'title',
                    index: 'title',
                ...
                },             
                {
                	...
                	editable: true,
                     ...
                     editoptions: {dataUrl: $("#PageContext").val()+'/CaseShowServlet?page=1&rows=500&flag=1'},
                ...
                }, 
                {
                	...
                	formatter: function (value, options, row) {
                        // 自定义单元格展示内容
                        return "<img src='/imagespath/"+row.imgname+"' width='155px' height='100px'/>";
                    }
                },
                {
                    name: 'handle',
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddCaseServlet",
            hidegrid: false,
        ...
        }).navGrid('#pager_list_2',{edit : false,add : true,del : false,search:false},
        		{
            ...
            afterSubmit:function (response) {
                ...
                if(status){
                    $.ajaxFileUpload({
                        url:$("#PageContext").val()+"/AddCasePicServlet",
                        fileElementId:"image",
                        data:{id:id},
                        type:"post",
                        success:function () {
                            $("#pager_list_2").trigger("reloadGrid")
                        }
                    });
                }
            }
