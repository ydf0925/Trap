...
function del(id) {
    ...
    var jsondata = {"id": id};
    $.ajax({
        url: $("#PageContext").val()+"/DelLinkServlet",
        type: "POST",
        data: jsondata,
        success: function (data, stutas) {
            ...
            if (data == "1") {
                ...
                successalert("", "删除友情链接成功");
            } else {
                errorsalert("", "删除友情链接失败");
            }
        },
        error: function () {
...
function modify() {
    ...
    $.ajax({
        url: $("#PageContext").val()+"/UpdateLinkServlet",
        type: "POST",
        data: $("#changefrom").serialize(),
        success: function (data) {
            if (data == 1) {
                successalert("", "友情链接修改成功");
                $("#table_list_2").trigger("reloadGrid");
                changedialogdismiss();
            } else {
                errorsalert("", "出现错误，请重试");
...
function buildgrid() {
    $("#table_list_2").jqGrid(
        {
            url: $("#PageContext").val()+"/LinkServlet",
            datatype: "json",
            mtype: "POST",
            height: "100%",
            autowidth: true,
            ...
            edit: false,
            addtext: 'Add',
            editurl: $("#PageContext").val()+"/AddLinkServlet",
            hidegrid: false,
